...
options.h = 68; % metres of max MLD, h of layer 2
% moving window range 
...
run NCP_Huang_planes.m
% get oxygen inventory change with time
run NCP_Huang_inventory.m
% calculate entrainment
run NCP_Huang_entrainment.m
% calculate advection term
run NCP_Huang_advection.m
% calculate air-sea exchange
run NCP_Huang_airseaexchange.m
% calculate diapycnal diffusion
run NCP_Huang_kz.m

%% calculate NCP and associated errors
% concatenate advection
ADV = [O2_adv.adv];
INVh1 = O2_inv.inv_h1(2:end-1)';
INVh2 = O2_inv.inv_h2(2:end-1)';
ASE = O2_ase.ASE(2:end-1);
ENT = [O2_ent.ent];
