...
% needed to calculate the advection term (testing whether change using MLD
% O2 only)

% created by MPH in Norwich, 14/11/2017
% modified by MPH in Sydney, 27/05/2020
...
for day = options.dayrange
    ...
    planes_loop(day).oxygen_planes.O2_MLD = vars.O2_MLD(planes_loop(day).day_selection_h & check_nan);
    ...
    planes_loop(day).oxygen_planes.const = ones(size(planes_loop(day).oxygen_planes.O2_MLD));
    ...
    planes_loop(day).oxygen_planes.coefficients_from_matrix = planes_loop(day).oxygen_planes.plane_matrix\planes_loop(day).oxygen_planes.O2_MLD';
    % fit function below gets same result as line above. 
    f = find(isnan(planes_loop(day).oxygen_planes.O2_MLD'));
    O2_MLD = planes_loop(day).oxygen_planes.O2_MLD'; O2_MLD(f) = [];
    xa = planes_loop(day).oxygen_planes.xa'; xa(f) = [];
    ya = planes_loop(day).oxygen_planes.ya'; ya(f) = [];
    plane_matrix = [xa, ya, ones(size(ya))]; % ignoring code below for now <---- TAKE NOTE OF THIS IF KEEP THIS EXPERIMENTAL CODE!!!!
    [planes_loop(day).oxygen_planes.O2fit, planes_loop(day).oxygen_planes.gof,~]  = fit(plane_matrix(:,1:2),O2_MLD,'poly11');
    ...
    planes_loop(day).oxygen_planes.O2_resid = planes_loop(day).oxygen_planes.O2_MLD - planes_loop(day).oxygen_planes.O2_predictions;
    planes_loop(day).oxygen_planes.O2_SSR = sum(planes_loop(day).oxygen_planes.O2_resid.^2);
    planes_loop(day).oxygen_planes.O2_SSE =  planes_loop(day).oxygen_planes.O2_predictions - mean(planes_loop(day).oxygen_planes.O2_MLD); 
    ...
    planes_loop(day).oxygen_planes.m_lon_standarderror = sqrt( nansum( ...
        (planes_loop(day).oxygen_planes.O2_MLD' - (planes_loop(day).oxygen_planes.m_0 + ...
        planes_loop(day).oxygen_planes.m_lon.*planes_loop(day).oxygen_planes.plane_matrix(:,1) )).^2) ) ...
        ./ length(planes_loop(day).oxygen_planes.O2_MLD);    
    ...
    planes_loop(day).oxygen_planes.plane_standarderror = sqrt( nansum( ...
        (planes_loop(day).oxygen_planes.O2_MLD' - planes_loop(day).oxygen_planes.O2fit(planes_loop(day).oxygen_planes.plane_matrix(:,1:2))).^2 ) ...
        ./ length(planes_loop(day).oxygen_planes.O2_MLD) ) ; 
    ...
    planes_loop(day).across_O2.S = planes_loop(day).oxygen_planes.O2_MLD(...
        planes_loop(day).oxygen_planes.lat > 43.2 & planes_loop(day).oxygen_planes.lat  <=43.3);
    planes_loop(day).across_O2.N = planes_loop(day).oxygen_planes.O2_MLD(...
        planes_loop(day).oxygen_planes.lat  > 43.43 & planes_loop(day).oxygen_planes.lat  < 43.5);
    planes_loop(day).across_O2.W = planes_loop(day).oxygen_planes.O2_MLD(...
        planes_loop(day).oxygen_planes.lon  > 7.64 & planes_loop(day).oxygen_planes.lon  < 7.725);
    planes_loop(day).across_O2.E = planes_loop(day).oxygen_planes.O2_MLD(...
