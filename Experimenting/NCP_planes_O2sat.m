...
for day = options.dayrange
    ...
    planes_loop(day).oxygen_planes.O2_sat = vars.O2_sat(planes_loop(day).day_selection_h & check_nan);
    ...
    planes_loop(day).oxygen_planes.coefficients_from_matrix = planes_loop(day).oxygen_planes.plane_matrix\planes_loop(day).oxygen_planes.O2_sat';
    % fit function below gets same result as line above. 
    [planes_loop(day).oxygen_planes.O2fit, planes_loop(day).oxygen_planes.gof,~]  = fit(planes_loop(day).oxygen_planes.plane_matrix(:,1:2),planes_loop(day).oxygen_planes.O2_sat','poly11');
    ...
    planes_loop(day).oxygen_planes.O2_resid = planes_loop(day).oxygen_planes.O2_sat- planes_loop(day).oxygen_planes.O2_predictions;
