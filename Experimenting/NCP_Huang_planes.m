...
testing =1;
if sum(~exist('planes_dens.mat','file') + testing ~= 0) >= 1
...
for day = options.dayrange
   ...
   planes_loop(day).day_selection = vars.t > (planes_loop(day).date_num-options.window) & ...
    ...
    planes_loop(day).means.MLD = nanmean(vars.MLD(planes_loop(day).day_selection));
    ...
    planes_loop(day).day_selection_h1 = vars.t > (planes_loop(day).date_num-options.window) & ...
        vars.t < (planes_loop(day).date_num+options.window) & ...
        vars.P <= planes_loop(day).means.MLD;  % only top h metres for most variables   
    planes_loop(day).day_selection_h2 = vars.t > (planes_loop(day).date_num-options.window) & ...
        vars.t < (planes_loop(day).date_num+options.window) & ...
        vars.P > planes_loop(day).means.MLD & vars.P <= options.h;  % only top h metres for most variables    
    ...
    % all plane fit variables require h, not h1 or h2
    ...
    planes_loop(day).means.O2_h1 = nanmean(vars.O2(planes_loop(day).day_selection_h1));  % mmol m^-3    
    planes_loop(day).means.O2_std_h1 = nanstd(vars.O2(planes_loop(day).day_selection_h1));  % mmol m^-3 
    planes_loop(day).means.O2_h2 = nanmean(vars.O2(planes_loop(day).day_selection_h2));  % mmol m^-3    
    planes_loop(day).means.O2_std_h2 = nanstd(vars.O2(planes_loop(day).day_selection_h2));  % mmol m^-3 
    ...
    planes_loop(day).means.O2_inv_h1 = planes_loop(day).means.O2_h1*planes_loop(day).means.MLD;  % mmol m^-2    
    planes_loop(day).means.O2_inv_h2 = planes_loop(day).means.O2_h1* ...
        (options.h - planes_loop(day).means.MLD);  % mmol m^-2    
...
% get depth of isopycnal (has to be some smoothing)
xgrid = [datenum(2016,03,9):1:datenum(2016,04,05)];
ygrid = [1028.5:0.01:1029.15];
checknans = isfinite(vars.depth) & isfinite(vars.t) & isfinite(vars.sigma0);
depth_dens_bin = bindata2(vars.depth(checknans),vars.t(checknans),...
    vars.sigma0(checknans),xgrid,ygrid);
for day = options.dayrange
    planes_loop(day).means.depth_of_isopycnal  = depth_dens_bin(51,day-8);
end

clear O2_dayUmb *dayPUmb ii bin_depth_check bn check* ...
    date_num day GPA GPA_* lat lon press_selection *_bin ...
    depth_dens_bin checknans xgrid ygrid
%% save file for later use
save('planes_dens','planes_loop');

%% if file already created
else
    disp(['Plane-fits | Loading previously created file containing plane-fits']);
    load('planes_dens.mat');
end
