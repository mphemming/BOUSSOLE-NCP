...
for day = options.dayrange
   ...
   planes_loop(day).day_selection_h_profs = profs.t > (planes_loop(day).date_num-options.window) & ...
    ...
    % advection
    check_nan = isfinite(vars.O2) & isfinite(vars.xa) & isfinite(vars.ya);
    planes_loop(day).oxygen_planes.O2_adv = vars.O2_MLD(planes_loop(day).day_selection_h_adv & check_nan);    
...
for n_bin = 1:numel(zgrid)
    ...
    % try 2 standard deviation range
%     O2(O2 < (nanmean(O2)-nanstd(O2)*2)) = NaN;
%     O2(O2 > (nanmean(O2)+nanstd(O2)*2)) = NaN;    
    check_nans = isfinite(O2);
    if numel(O2(check_nans)) > 20
        % prepare for fit
        ...
        % same again but for Robust fitting
