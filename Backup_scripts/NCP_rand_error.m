...
simulated_errors.O2_ASE.calibration_1 = pearsrnd(0,3.3,0,3,100000,1); % mmol m-3
simulated_errors.O2_ASE.calibration_2 = pearsrnd(0,3.3,0,3,100000,1); % mmol m-3
...
for ts = 1:numel(O2_ase.ASE)
    for n = 1:100000
        simulated_errors.O2_ASE.ASE_sim(ts,n) = ((O2_ase.KO2(ts))/100*24).*simulated_errors.O2_ASE.gas(n) ...
            .*((simulated_errors.O2_ASE.O2_surf(ts)+simulated_errors.O2_ASE.calibration_1(n) - ...
            (O2_ase.O2_saturation(ts)+simulated_errors.O2_ASE.calibration_2(n)+errors.O2_ASE.O2_sat_sim(ts,n) .* O2_ase.bub(ts)))) .*O2_ase.correction(ts) ;
    end
end

% example of using monte carlo function

% gas_error = generateMCparameters('gaussian',[0,20],'plot',true);
% calibration_O2surf_error = generateMCparameters('gaussian',[0,3.3],'plot',true);
% calibration_O2sat_error = generateMCparameters('gaussian',[0,3.3],'plot',true);
% 
% paramMatrix = [gas_error; calibration_O2surf_error; calibration_O2sat_error];
% funToProp = @(x) x(1) * (x(2) - x(3))
...
% get standard errors from bootstrapping
% for day = options.dayrange-8
%     bt(day).vals = bootstrp(100000,@(x)[mean(x)],planes_loop(day+8).oxygen_planes.O2_profs);
%     errors.invO2(day).std_errors = nanstd(bt(day).vals);
% end



for day = options.dayrange-8
    % when using propogated errors
    errors.invO2(day).O2_std = errors.invO2(day).errors_edge;
    % when using standard errors
%     errors.invO2(day).O2_std = means_struct(day).O2_standard_error_profs;
...
%         + simulated_errors.O2_ASE.ASE_sim(ts,:) - ENT(ts);
    ...
    simulated_errors.O2_NCP(ts,:) = simulated_errors.ADV(ts).ADV_estimate' ...
        + simulated_errors.O2_ASE.ASE_sim(ts,:);    
    % old method ADV: simulated_errors.O2_ADV.ADV_std(ts,:)
%     simulated_errors.O2_NCP(ts,:) = ' + simulated_errors.ADV(ts).ADV_estimate' ...
%         + simulated_errors.O2_ASE.ASE_sim(ts,:) - ENT(ts);    
end

% if using propogated errors for inventory change
for ts = 2:26
    NCP_error(ts) = sqrt( (errors.invO2(ts).errors_edge)^2 + nanstd(simulated_errors.O2_NCP(ts,:))^2);
