...
% Modified by BYQ to account for NaNs and to permute the matrix at the end.
% TODO : The accumarray function has similar accumulation behavior to that of sparse. accumarray groups data into bins using n-dimensional subscripts, but sparse groups data into bins using 2-D subscripts. accumarray adds elements that have identical subscripts into the output by default, but can optionally apply any function to the bins. sparse only adds elements that have identical subscripts into the output.
